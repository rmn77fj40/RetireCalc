...
const COLORADO_STATE_TAX_RATE_2024 = 0.0425;
const COLORADO_STATE_TAX_RATE_2025_ONWARDS = 0.040;
...
function calculateColoradoStateTax(totalIncomeForStateTax, robAge, sonjaAge, currentYear) {
    if (totalIncomeForStateTax <= 0) return 0;

    let coloradoStateTaxRate = (currentYear >= 2025) ? COLORADO_STATE_TAX_RATE_2025_ONWARDS : COLORADO_STATE_TAX_RATE_2024;
    ...
    let incomeAfterCODRetirementDeduction = Math.max(0, totalIncomeForStateTax - retirementIncomeDeduction);
...
function calculateRetirement() {
    ...
    for (let year = currentYear; year <= currentYear + (planToAge - Math.min(robInitialAge, sonjaInitialAge) + 5); year++) {
        const robAge = year - currentYear + robInitialAge;
        const sonjaAge = year - currentYear + sonjaInitialAge;

        if (robAge > planToAge && sonjaAge > planToAge && currentSavings > 0) {
            if (robAge > planToAge + 5 && sonjaAge > planToAge + 5 && currentSavings < 1000) break;
        }
        ...
        let annualColoradoStateTaxes = calculateColoradoStateTax(initialTotalGrossIncomeNominal, robAge, sonjaAge, year);
        ...
        if (cashFlowBeforeWithdrawals < 0) {
            ...
            while (iterations < MAX_ITERATIONS) {
                // Estimate withdrawals needed (deficit + estimated tax on previous withdrawal)
                // For a simpler model, we can just assume all the deficit needs to be covered by withdrawals.
                // The new taxes will increase the deficit, leading to more withdrawals, and more taxes...
                // A simpler approach: Add the current deficit to withdrawals, then recalculate taxes.
                ...
                let newColoradoTax = calculateColoradoStateTax(incomeWithPotentialWithdrawalsCO, robAge, sonjaAge, year);
